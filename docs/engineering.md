...
本规范适用于 AI Writer 项目的工程化实践，包括新增模块流程、共享组件/hook 开发规范、数据库设计、CSS 变量管理、代码质量门禁、性能预算、数据增长策略、模块拆分指南。
...
| # | 校验项 | 自动化 | 手动 |
|---|--------|--------|------|
| E-1 | 新模块按 Checklist 执行（DB→Types→API→UI→注册） | Code Review | — |
| E-2 | 新表有 id/book_id/timestamps | 迁移脚本审查 | — |
| E-3 | 共享组件在 shared/ui/ 且无业务逻辑 | Code Review | — |
| E-4 | 共享 hook 无 app/pages/ 依赖 | 搜索 import | — |
| E-5 | CSS 变量在 globals.css 定义 | 代码检查 | — |
| E-6 | 主题变量同步更新 ThemeColors + themes.ts | Code Review | — |
| E-7 | 提交前 typecheck + lint + build | CI / 本地验证 | — |
| E-8 | 文件拆分阈值已遵循（组件200行/Hook150行） | 代码行数检查 | — |

## 违规整改方案

| 违规 | 整改方式 | 时限 |
|------|---------|------|
| 新模块未按 Checklist 执行 | 补充缺失步骤 | 当前迭代 |
| 新表缺少必要字段 | 补充迁移添加缺失字段 | 当前迭代 |
| 共享组件包含业务逻辑 | 提取业务逻辑到 hooks/pages | 当前迭代 |
| 共享 hook 依赖 pages/ | 重构依赖方向 | 当前迭代 |
| CSS 变量未在 globals.css 定义 | 补充定义 | 当前迭代 |
| 文件过大未拆分 | 按阈值拆分为子文件 | 当前迭代 |
